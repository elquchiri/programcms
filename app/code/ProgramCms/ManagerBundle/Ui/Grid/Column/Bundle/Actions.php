...
namespace ProgramCms\ManagerBundle\Ui\Grid\Column\Bundle;
...
/**
 * Class Actions
 * @package ProgramCms\ManagerBundle\Ui\Grid\Column\Bundle
 */
    ...
    public function prepare()
    {
        ...
        if($this->hasData('source')) {
            ...
            foreach($dataSourceData as $rowData) {
                $urlRouteName = $rowData->getStatus() === false ? 'manager_bundle_activate' : 'manager_bundle_disable';
                $actions = [
                    [
                        'label' => $rowData->getStatus() === false ? $this->trans('Activate') : $this->trans('Disable'),
                        'url' => $this->url->getUrlByRouteName($urlRouteName, ['id' => $rowData->getEntityId()]),
                        'type' => 'url'
                    ]
                ];
